...
strategy_performance = {s: [] for s in [
    "trend_strategy","pullback_strategy","mean_reversion","bb_mean_reversion",
    "volatility_breakout","rsi_reversion","momentum_strategy",
    "breakout_mean_reversion","trend_following"
]}
peak_equity = 0  # for max drawdown
...
def get_strategy_weight(name):
    ...
    sharpe = np.mean(history)/(np.std(history)+1e-6)
    return max(0.5,min(3,sharpe))
...
def process_instrument(instrument):
    ...
    if signal=="BUY" and current_position<=0: place_order(instrument, units, atr)
    if signal=="SELL" and current_position>=0: place_order(instrument, -units, atr)
...
# =========================
# HIGHER TIMEFRAME REFRESH THREAD
# =========================
def refresh_higher_timeframes():
    while True:
        for instrument in INSTRUMENTS:
            market_data[instrument+"_H1"] = compute_features(get_candles(instrument,"H1"))
            market_data[instrument+"_D"] = compute_features(get_candles(instrument,"D"))
        time.sleep(3600)
