...
BASE_RISK = 0.01          # default 1% per trade
MAX_RISK = 0.03           # up to 3% per trade in high volatility
...
signal_history = {pair: [] for pair in INSTRUMENTS}  # last few signals
MIN_SIGNAL_CONFIRM = 3  # number of agreeing signals required
MIN_STRATEGY_CONFIRM = 3
MIN_ATR = 0.0005
...
def multi_timeframe_regime(instrument):
    ...
    # Skip trades if higher timeframes are flat
    if rd in ["range","compression"]: return None
    if r1h in ["range","compression"]: return None
    return r5
...
def position_size(balance, atr, df):
    risk_per_trade = BASE_RISK
    atr_mean = df["atr"].rolling(50).mean().iloc[-1]
    if df["atr"].iloc[-1] > atr_mean * 1.2:
        risk_per_trade = min(MAX_RISK, BASE_RISK * 2)
    stop_distance = atr*2
    port_vol = portfolio_volatility()
    vol_scale = TARGET_VOL/(port_vol+1e-6)
    units = (balance * risk_per_trade) / stop_distance * vol_scale
    return int(units)

# =========================
# HIGH-VOLATILITY SESSION FILTER
# =========================
def in_active_session():
    utc_hour = datetime.datetime.utcnow().hour
    return 13 <= utc_hour <= 17  # London/New York overlap

# =========================
# VOTING ENGINE WITH FALSE-SIGNAL FILTER
# =========================
def enhanced_voting_engine_filtered(df, regime, instrument):
    ...
    votes = np.array(votes); weights = np.array(weights)
    if np.sum(votes>0) >= MIN_STRATEGY_CONFIRM: direction = 1
    elif np.sum(votes<0) >= MIN_STRATEGY_CONFIRM: direction = -1
    else: direction = 0

    atr = df["atr"].iloc[-1]
    ema_slope = df["ema20"].iloc[-1] - df["ema20"].iloc[-5]
    if abs(ema_slope) < EMA_SLOPE_THRESHOLD or atr < MIN_ATR:
        direction = 0

    # Smooth last 5 signals
    history = signal_history[instrument]
    if direction == 1: history.append("BUY")
    elif direction == -1: history.append("SELL")
    else: history.append("HOLD")
    if len(history) > 5: history.pop(0)
    signal_history[instrument] = history

    if history.count("BUY") >= MIN_SIGNAL_CONFIRM: return "BUY"
    if history.count("SELL") >= MIN_SIGNAL_CONFIRM: return "SELL"
    return "HOLD"
...
def place_order(instrument, units, atr):
    ...
    data = {"order":{"instrument":instrument,"units":str(units),"type":"LIMIT",
                      "price":str(round(price,5)),"timeInForce":"FOK",
...
# PROCESS INSTRUMENT
# =========================
def process_instrument_aggressive_filtered(instrument):
    ...
    signal = enhanced_voting_engine_filtered(df, regime, instrument)
    atr = df["atr"].iloc[-1]
    balance = get_account_balance()
    units = position_size(balance, atr, df)
...
for tick in client.request(r):
    ...
    process_instrument_aggressive_filtered(instrument)
